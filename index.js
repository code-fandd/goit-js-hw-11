...
</div>`}let g=new u(".gallery a",{captionsData:"alt",captionDelay:250});const s={formEl:document.querySelector("#search-images"),galleryEl:document.querySelector(".gallery"),loader:document.querySelector(".loader")};s.formEl.addEventListener("submit",t=>{t.preventDefault();const a=t.target.elements.query.value.trim();s.loader.style.display="block",s.galleryEl.innerHTML="",m(a).then(o=>{if(!o)return;const i=o.map(d).join("");s.galleryEl.insertAdjacentHTML("beforeend",i),g.refresh()}).catch(o=>{console.error("Error fetching images:",o),l.error({message:"Something went wrong. Please try again later.",position:"topRight"})}).finally(()=>{s.loader.style.display="none"}),t.target.reset()});
